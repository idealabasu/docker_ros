...
class NatNetClient:
    def __init__(
        ...
        # This should match the multicast address listed in Motive's streaming
        # settings.
        ...
        # Set this to a callback method of your choice to receive
        # data at each frame.
        self.listener = listener

        # NatNet stream version. This will be updated to the actual version the
        # server is using during initialization.
    ...
    def __unpackRigidBody(self, data, buffer=None):
        offset = 0
        ...
        if buffer is not None:
            buffer.append([id, pos, rot])

        # RB Marker Data ( Before version 3.0.  After Version 3.0 Marker data
        # is in description )
        if (self.__natNetStreamVersion[0] <
                3 and self.__natNetStreamVersion[0] != 0):
            # Marker count (4 bytes)
            markerCount = int.from_bytes(
    ...
    def __unpackMocapData(self, data):
        ...
        frameNumber = int.from_bytes(
            data[offset:offset + 4], byteorder='little')
        offset += 4
        trace("Frame #:", frameNumber)

        # Marker set count (4 bytes)
        markerSetCount = int.from_bytes(
            data[offset:offset + 4], byteorder='little')
        offset += 4
        ...
        for i in range(0, markerSetCount):
            # Model name
            modelName, separator, remainder = bytes(
                data[offset:]).partition(b'\0')
        ...
        for i in range(0, rigidBodyCount):
            offset += self.__unpackRigidBody(
                data[offset:],
                buffer=rigid_body_buffer
            )

        # Version 2.1 and later
        skeletonCount = 0
        if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >
                0) or self.__natNetStreamVersion[0] > 2):
        ...
        labeled_marker_buffer = []
        if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >
                3) or self.__natNetStreamVersion[0] > 2):
            ...
            for i in range(0, labeledMarkerCount):
                ...
                # Version 2.6 and later
                if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >=
                        6) or self.__natNetStreamVersion[0] > 2 or major == 0):
                    param, = struct.unpack('h', data[offset:offset + 2])
                    offset += 2
                ...
                labeled_marker_buffer.append([id, pos])

        # Force Plate data (version 2.9 and later)
        if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >=
                9) or self.__natNetStreamVersion[0] > 2):
        ...
        if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >=
                11) or self.__natNetStreamVersion[0] > 2):
            deviceCount = int.from_bytes(
                data[offset:offset + 4], byteorder='little')
            offset += 4
            trace("Device Count:", deviceCount)
            for i in range(0, deviceCount):
                # ID
                deviceID = int.from_bytes(
                    data[offset:offset + 4], byteorder='little')
                offset += 4
        ...
        offset += 4
        timecodeSub = int.from_bytes(
            data[offset:offset + 4], byteorder='little')
        offset += 4

        # Timestamp (increased to double precision in 2.7 and later)
        if ((self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >=
                7) or self.__natNetStreamVersion[0] > 2):
    ...
    def __unpackRigidBodyDescription(self, data):
        ...
        # Version 3.0 and higher, rigid body marker information contained in
        # description
        if (self.__natNetStreamVersion[0] >=
                3 or self.__natNetStreamVersion[0] == 0):
            markerCount = int.from_bytes(
                data[offset:offset + 4], byteorder='little')
            offset += 4
    ...
    def __processMessage(self, data):
        ...
        if (messageID == self.NAT_FRAMEOFDATA):
            ...
            if (packetSize == 4):
                ...
                message, separator, remainder = bytes(
                    data[offset:]).partition(b'\0')
    ...
    def sendCommand(self, command, commandStr, socket, address):
        ...
        if (command == self.NAT_REQUEST_MODELDEF or command ==
    ...
    def run(self):
        ...
        dataThread = Thread(
            target=self.__dataThreadFunction,
...
if __name__ == "__main__":
    ...
    def listener(timestamp, labeled_marker_buffer, rigid_body_buffer):
        print(timestamp, labeled_marker_buffer, rigid_body_buffer)
